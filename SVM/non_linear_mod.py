...
def handler(decision_function_shape_in,gamma_in,dataset_in):
	...
	return [accr_rbf_svc_score,accr_polynomial_svc_2_score,accr_polynomial_svc_3_score,accr_sigmoid_svc_score]
def non_linear_train(dataset_in):
	...
	np.savetxt('non linear one versus one.csv',result_matrix,delimiter=',')
	############################################################################################################
	...
	for j in gamma_values:
		...
		result_matrix.append(temp)
	np.savetxt('non linear one versus rest.csv',result_matrix,delimiter=',')
	#############################################################################################################
